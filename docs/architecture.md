...
用户始终从会话进入业务流程，并通过模式区分即时问答与产物型任务。Work 模式的完成标准是产出一个 Artifact；阶段进度、思考内容和工具调用只描述执行过程。

| 模式 | 使用目的 | 输入 | 输出 | 约束 |
| --- | --- | --- | --- | --- |
| Chat | 普通问答、文件问答、联网搜索 | 文本、可选文件、可选 Skill | 流式回答、来源、推荐问题 | 不允许携带 `agent_type` |
| Work | 完成任务并产出一个 Artifact | 任务描述、Agent 类型、可选文件或 Skill | 阶段进度、最终回答、一个最终 Artifact（图片、研究报告或 PPT/PPTX） | 必须选择一个有效 Agent |
...
- 深度研究：生成研究主题 -> 用户确认 -> Deep Agents“计划 -> 执行 -> 评估”循环 -> 汇总报告 -> 校验引用 -> 保存 Markdown 报告。
...
- 按标题和消息正文搜索会话，返回命中摘要。
- 接收并复验选择器提交的 Skill 标识；未提交时按任务内容自动匹配一个 Skill，但忽略 `@Skill` 形式的文本。
...
Skill Service 提供名称、描述、指令和启停状态的 CRUD。选择器提交的 Skill 会直接加载完整指令，服务端复验每个 `skill_id` 均存在且已启用；未提交时根据名称与描述自动匹配一个 Skill。Chat 与 Work 均可显式提交多个 Skill，同时 Agent 字段仍为单值。
...
外层 LangGraph：生成研究主题 -> 对话与修订主题 -> 等待用户确认
              -> Deep Agents 计划 -> 执行 -> 评估
                    ^                    |
                    |-- 证据仍不足 -----|
              -> 汇总报告 -> 证据与引用校验 -> 保存 Markdown

Deep Agents：每轮规划 -> 搜索与提取 -> 必要的固定子 Agent 委派
            -> 证据整理 -> 覆盖度与缺口评估
```
