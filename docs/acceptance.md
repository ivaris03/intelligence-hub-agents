...
| 13 | Memory 管理与记住/忘记 | 显式命令数据库回执、相关注入、CRUD/开关路径 | 设置页管理；“请记住”立即显示回执并可在设置页看到 | 通过 |
| 14 | 闲置提炼、游标与总开关 | `test_idle_memory_refinement_is_cursor_based_safe_and_switchable` 覆盖 30 分钟、去重游标、冲突拒绝和关闭开关 | Memory 开关关闭时输入禁用，重新开启立即恢复 | 通过 |
