...
| MEMORY-04 | 闲置 6 小时入队并在用户本地午夜批量提炼 | 使用持久化待处理队列与游标避免遗漏和重复；敏感、含糊和冲突内容不自动写入 | MEMORY-01/03 |
...
| RES-01 | 研究主题确认、对话修订、外层 LangGraph 与预算控制 | 无论需求是否明确都先生成结构化研究主题并等待确认；等待期间可对话并产生主题新版本，确认前不搜索，确认后由外层图管理阶段、取消、总耗时和共享搜索次数 | RUN-01/WEB-01 |
| RES-02 | Deep Agents 研究循环 | 通过“计划-执行-评估”循环拆解问题、调整计划、搜索和整理证据；证据不足且预算允许时进入下一轮 | RES-01 |
