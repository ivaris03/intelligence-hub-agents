...
| `QWEN_THINKING_BUDGET` | 前端“中”思考强度的基准预算，默认 1024 tokens；“低”为 0.5 倍，“高”为 4 倍 |
...
5. 新建会话时先选择 Chat 或 Work；选择模式不会立即落库，发送第一句话后才以该内容命名并创建会话。创建后类型不可切换；两种会话统一显示在侧栏，但不共用历史、文件或上下文。Work 会话中可选择图片、演示或研究 Agent。
